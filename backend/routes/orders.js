...
router.post('/', auth, async (req, res) => {
  try {
    const { items, subtotal, gstEnabled, gstRate, gst, total, note, paymentMethod, upiOwner } = req.body;
    ...
    const order = await Order.create({
      ...
      subtotal: calcSubtotal, gstEnabled: !!gstEnabled, gstRate: gstRate || 0, gst: calcGst, total: calcTotal,
      note: note || '',
      paymentMethod: paymentMethod === 'online' ? 'online' : 'cash',
      upiOwner: paymentMethod === 'online' ? (upiOwner || '') : '', // ✅ only save for online
    });
...
router.get('/', auth, async (req, res) => {
  try {
    const { date, from, to, page = 1, limit = 50, month } = req.query;
